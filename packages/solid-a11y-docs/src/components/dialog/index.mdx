...
(additional context and information) components. Instead of `<Label />`, one may use the
`<DialogTitle />` which uses a better default tag of `h1` but is otherwise identical to `<Label />`.

<CodeSampleShowcase source={ProcessedWithCustomLabelExample} />

## Focus Management

It is a good idea to ensure that a dialog has at least one focusable element contained within it. A
very common pattern will be a button to dismiss the dialog. By default, `<Dialog />` will focus the
first item within itself and also "trap" focus within the dialog meaning that `Tab`/`Shift+Tab` will
round robin within the dialog's focusable elements.
...
key-presses do not disturb the user. For instance, in a delete confirmation dialog, it makes sense
to focus the cancel button rather than the affirmative button.
